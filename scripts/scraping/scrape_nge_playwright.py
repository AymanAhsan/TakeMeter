"""
scrape_nge_playwright.py — scrape r/NeonGenesisEvangelion Discussion flair
posts and top comments using Python Playwright (no Reddit API).

Target: 200 rows (posts + comments) appended to dataset.csv.
Run:
    python scrape_nge_playwright.py
"""
...
FLAIR_URL = (
    "https://www.reddit.com/r/NeonGenesisEvangelion/"
    "?f=flair_name%3A%22Discussion%22"
)
...
def scrape_post(page, url: str, existing_ids: set[str]) -> list[dict]:
    ...
    pid        = post_el.get_attribute("id") or ""
